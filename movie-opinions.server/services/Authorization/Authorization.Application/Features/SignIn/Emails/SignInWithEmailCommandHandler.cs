...
namespace Authorization.Application.Features.SignIn.Emails
{
    public class SignInWithEmailCommandHandler : IRequestHandler<SignInWithEmailCommand, Result<SignInResult<Guid>>>
    {
        ...
        public SignInWithEmailCommandHandler(SignInFlowCoordinator signInFlowCoordinator)
        {
            _signInFlowCoordinator = signInFlowCoordinator;
        }

        public async Task<Result<SignInResult<Guid>>> Handle(SignInWithEmailCommand command, CancellationToken cancellationToken = default)
        {
