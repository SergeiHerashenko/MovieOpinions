...
{
    public class BlockCheck : IAccessStep<ISignInMarker>
    {
        ...
        public Task<Result> ExecuteAsync(User user, CancellationToken cancellationToken = default)
        {
            ...
            return Task.FromResult(Result.Failure(UserErrors.UserIsBlocked<BlockCheck>(banSession.GetRemainingTime(_clock.UtcNow))));
        }
    }
}
