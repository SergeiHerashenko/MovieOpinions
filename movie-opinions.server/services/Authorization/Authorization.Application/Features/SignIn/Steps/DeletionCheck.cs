...
{
    public class DeletionCheck : IAccessStep<ISignInMarker>
    {
        ...
        public Task<Result> ExecuteAsync(User user, CancellationToken cancellationToken = default)
        {
            ...
            if (deletion.Value.Status == DeletionStatus.PermanentlyDeleted)
                return Task.FromResult(Result.Failure(UserErrors.UserIsPermanentlyDeleted<DeletionCheck>()));

            return Task.FromResult(Result.Failure(UserErrors.UserIsDeleted<DeletionCheck>(deletion.Value.RestoreUntil)));
        }
    }
}
