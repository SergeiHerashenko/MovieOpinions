...
{
    public sealed class TooLongDomainPartRule : IValidationRule<string, ValidationRestoreFailure>
    {
        private const int MAX_LENGTH_EMAIL_DOMAIN = 40;
        ...
        public ValidationRestoreFailure? Validate(string value)
        {
            ...
            if (value.Length <= MAX_LENGTH_EMAIL_DOMAIN)
                return null;

            return new ValidationRestoreFailure()
            {
                Error = EmailErrors.TooLongDomainPart<EmailDomainPart>(value),
