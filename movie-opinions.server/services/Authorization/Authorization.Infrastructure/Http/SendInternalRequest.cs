...
namespace Authorization.Infrastructure.Http
{
    public class SendInternalRequest : ISendInternalRequest
    {
        ...
        public async Task<HttpResult<TResponse>> SendAsync<TBody, TResponse>(InternalRequest<TBody> internalRequest, CancellationToken cancellationToken = default)
        {
            try
            {
        ...
        public async Task<HttpResult<TResponse>> SendAsync<TResponse>(InternalRequest internalRequest, CancellationToken cancellationToken = default)
        {
            try
            {
                // 1. Створюємо іменований клієнт
                ...
                HttpResponseMessage response = internalRequest.Method.Method.ToUpper() switch
                {
                    "POST" => await client.PostAsync(internalRequest.Endpoint, null),
                    "GET" => await client.GetAsync(internalRequest.Endpoint),
                    "PUT" => await client.PutAsync(internalRequest.Endpoint, null),
                    "DELETE" => await client.DeleteAsync(internalRequest.Endpoint),
                    _ => throw new NotSupportedException($"The method {internalRequest.Method} is not supported.")
                };
