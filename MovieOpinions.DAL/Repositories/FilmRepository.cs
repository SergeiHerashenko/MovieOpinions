...
namespace MovieOpinions.DAL.Repositories
{
    public class FilmRepository : IFilmRepository
    {
        ...
        public async Task<BaseResponse<List<Film>>> GetMovieGenre(IEnumerable<int> Genre)
        {
            List<Film> SelectFilm = new List<Film>();
            ...
            using(var conn = new NpgsqlConnection(connect.ConnectMovieOpinionsDataBase()))
            {
                try
                {
                    ...
                    string GenreCondition = string.Join(", ", Genre);

                    using(var GetFilmByGenre = new NpgsqlCommand(
                        "SELECT " +
                            "Film_Table.id_film, " +
                            "Film_Table.name_film, " +
                            "Film_Table.year_film, " +
                        ...
                        "LEFT JOIN " +
                            "Film_Rating ON Film_Table.id_film = Film_Rating.id_film " +
                        "WHERE " +
                            $"Film_Genre.id_genre IN ({GenreCondition}) " +
                        "GROUP BY " +
                            "Film_Table.id_film, " +
                            "Film_Table.name_film, " +
                            "Film_Table.year_film, " +
                            "Film_Table.description_film", conn))
                    {
                        using(var Reader = await GetFilmByGenre.ExecuteReaderAsync())
                        {
                            while (Reader.Read())
                            {
                                Film film = new Film()
        ...
        public async Task<BaseResponse<List<Film>>> GetMovieYear(IEnumerable<string> Year)
        {
            List<Film> SelectYearFilm = new List<Film>();
            ...
            using (var conn = new NpgsqlConnection(connect.ConnectMovieOpinionsDataBase()))
            {
                try
                {
                    ...
                    var WhereClause = GenerateWhereClause(Year);
        ...
        {
            List<Film> SearchFilm = new List<Film>();
            ...
            using (var conn = new NpgsqlConnection(connect.ConnectMovieOpinionsDataBase()))
            {
                try
                {
                    ...
                    using(var GetSearchResult = new NpgsqlCommand(
                        "WITH " +
                        "FilmsWithActor AS " +
                        "( " +
                            "SELECT " +
                            ...
                            "LEFT JOIN " +
                                "Actor_Table ON Film_Actor.id_actor = Actor_Table.id_actor " +
                            "WHERE " +
                                "CONCAT(Actor_Table.first_name_actor, ' ', Actor_Table.last_name_actor) ILIKE @SEARCHTEXT " +
                                "OR " +
                                "Film_Table.name_film ILIKE @SEARCHTEXT " +
                        ") " +
                        "SELECT " +
                            "Film_Table.id_film, " +
                            "Film_Table.name_film, " +
                        ...
                        "LEFT JOIN " +
                            "Film_Rating ON Film_Table.id_film = Film_Rating.id_film " +
                        "WHERE " +
                            "Film_Table.id_film IN (SELECT id_film FROM FilmsWithActor) " +
                        "GROUP BY " +
                            "Film_Table.id_film, " +
                            "Film_Table.name_film," +
                            "Film_Table.year_film, " +
                            "Film_Table.description_film;", conn))
                    {
                        GetSearchResult.Parameters.AddWithValue("@SEARCHTEXT", "%" +  PartialName + "%");

                        using(var Reader = await GetSearchResult.ExecuteReaderAsync())
                        {
                            while (Reader.Read())
                            {
        ...
        private string GenerateWhereClause(IEnumerable<string> YearFilm)
        {
            var conditions = new List<string>();
            foreach (var yearFilter in YearFilm)
            {
                if (yearFilter.Contains('-'))
                {
                    var years = yearFilter.Split('-');
                    conditions.Add($"(Film_Table.year_film >= {years[0]} AND Film_Table.year_film <= {years[1]})");
                }
                else
                {
                    conditions.Add($"Film_Table.year_film = {yearFilter}");
                }
            }
